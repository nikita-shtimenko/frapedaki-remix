...
export function AlertTitle({
  ...
  return (
    <Headless.DialogTitle
      ...
      className={clsx(
        className,
        "text-balance text-center text-base/6 font-semibold text-zinc-950 dark:text-white sm:text-wrap sm:text-right sm:text-sm/6",
      )}
    />
  );
}
...
>) {
  return (
    <Headless.Description
      as={Text}
      {...props}
      className={clsx(className, "mt-2 text-pretty text-center sm:text-right")}
    />
  );
}
...
export function AlertActions({
  ...
  return (
    <div
      ...
      className={clsx(
        className,
        "mt-6 flex flex-col-reverse items-center justify-end gap-3 *:w-full sm:mt-4 sm:flex-row-reverse sm:*:w-auto",
      )}
    />
  );
}
