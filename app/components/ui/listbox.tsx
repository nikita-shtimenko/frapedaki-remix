      ...
      >
        <Headless.ListboxSelectedOption
          ...
          className={clsx([
            ...
            "min-h-11 sm:min-h-9",
            // Horizontal padding
            "pl-[calc(theme(spacing.7)-1px)] pr-[calc(theme(spacing[3.5])-1px)] sm:pr-[calc(theme(spacing.3)-1px)]",
            // Typography
            "text-right text-base/6 text-zinc-950 placeholder:text-zinc-500 dark:text-white sm:text-sm/6 forced-colors:text-[CanvasText]",
            // Border
      ...
      {({ selectedOption }) => {
        ...
        return (
          <div
            className={clsx(
              // Basic layout
              "group/option grid cursor-default grid-cols-[theme(spacing.5),1fr] items-baseline gap-x-2 rounded-lg py-2.5 pl-3.5 pr-2 sm:grid-cols-[theme(spacing.4),1fr] sm:py-1.5 sm:pl-3 sm:pr-1.5",
              // Typography
...
export function ListboxLabel({
  ...
  return (
    <span
      ...
      className={clsx(
        className,
        "mr-2.5 truncate first:mr-0 sm:mr-2 sm:first:mr-0",
      )}
    />
  );
}
